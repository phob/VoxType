...
grep_score 6 grep -R "provisionalText = undefined" -n src/shared/transcript-turns.ts >/dev/null
grep_score 6 bash -c 'test $(grep -c "provisionalText = undefined" src/shared/transcript-turns.ts) -ge 2'
...
grep_score 6 grep -R "buildOpenAiAuthorizationHeader" -n src/main/openai-asr-provider.ts >/dev/null
grep_score 6 grep -R 'return `Bearer ${apiKey.trim()}`' -n src/main/openai-asr-provider.ts >/dev/null
...
grep_score 6 grep -R "child.once(\"close\"" -n src/main/windows-helper-service.ts >/dev/null
...
grep_score 6 grep -R "lastRealtimeCloudSessionErrorTtlMs" -n src/main/index.ts >/dev/null
grep_score 6 grep -R "recordedAtMs" -n src/main/index.ts >/dev/null
...
grep_score 6 grep -R "unknown parameter" -n src/shared/openai-errors.ts >/dev/null
grep_score 6 grep -R "unsupported" -n src/shared/openai-errors.ts >/dev/null
