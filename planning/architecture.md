...
- Hold-to-dictate uses a separate persisted hotkey. Electron registers the key-down activation, then the native Windows helper waits for the physical key combination to be released because Electron's global shortcut API does not provide a global key-up event.
