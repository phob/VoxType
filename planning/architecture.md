...
- Initial settings include model directory, insertion mode, app profiles, offline mode, developer mode, Windows login startup, start minimized, automatic update checks, clipboard restoration, remote typing delay, remote typing chunk size, configurable hotkeys, optional automatic system-audio mute while recording, global recording-coordination mode and hotkeys, recorder capture mode, and Silero VAD trimming controls.
...
- Release-mode recording ignores hidden developer-only recording knobs that can degrade dictation quality. Unless Developer Mode is enabled, recording uses shared Handy-parity capture and VAD on, even if stale persisted developer settings say otherwise.
