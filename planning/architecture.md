...
- `src/renderer/src/vad-trimmer.ts` runs the initial Silero VAD pass after recording stops. It uses `@ricky0123/vad-web` with ONNX Runtime Web/WASM and bundles the Silero ONNX model plus ONNX WASM assets into the renderer output. It now rebuilds audio from original sample ranges, merges nearby speech ranges so short natural pauses remain intact, and crossfades joins where longer silent ranges are removed. VAD trims audio for Whisper but does not automatically end the recording session.
