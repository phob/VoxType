...
Context Engine
  local context ranking, prompt-pack construction, confidence-scored corrections,
...
audio
  ...
  -> context engine prompt-pack selection
  -> ASR engine
  -> transcript normalization
  -> context engine dictionary/OCR corrections
  -> optional formatting
  -> insertion strategy
  -> target app
```
...
- Initial settings include model directory, insertion mode, app profiles, offline mode, a debug-view preference for local builds, Windows login startup, start minimized, automatic update checks, clipboard restoration, remote typing delay, remote typing chunk size, configurable hotkeys, optional automatic system-audio mute while recording, global recording-coordination mode and hotkeys, recorder capture mode, and Silero VAD trimming controls.
- Electron main applies the persisted Windows startup setting through the platform login-item API when settings are loaded, updated, or reset. `startWithWindows` controls automatic launch at sign-in, while `startMinimized` controls whether the main window hides to the tray after startup in packaged release builds. Unpackaged development builds, including preview runs, ignore `startMinimized` so the app window stays visible while iterating.
- Electron main owns automatic update checks. When enabled, VoxType checks once at startup and then roughly hourly. If the startup check finds an update, it reveals the main window even when Start minimized is enabled; later periodic checks update status silently.
- Electron main uses Electron's single-instance lock so builds that include the guard cannot start duplicate VoxType app instances. A second guarded launch restores/focuses the existing main window. This guard is intentionally not supplemented by process-name scanning for older releases while only the maintainer runs development builds; older unguarded releases can still run beside a guarded dev build.
- The renderer now defaults to a release-style dictation home for ordinary users and keeps the prior dense diagnostic interface behind `debugViewEnabled`, which is only offered in developer builds.
- The renderer app shell keeps lifecycle state, IPC subscriptions, and action wiring in `App.tsx`, while release/dev view projections such as setup readiness, Cloud Dictation status labels, filtered model lists, and update-button state live behind `src/renderer/src/app/app-derived-state.ts`. Shared renderer state shape and overlay defaults live in `src/renderer/src/app/app-state.ts`.
...
- The default dictation recorder uses shared CPAL capture through a warmed `native/windows-helper record-wav-session` process. The helper opens the selected input device, chooses a native/default sample format and rate, converts to mono, resamples to 16 kHz with Rubato FFT resampling, applies optional Silero VAD v4 through `vad-rs`, emits a ready event after `stream.play()`, accepts explicit start/stop/shutdown commands, resets VAD on start, drains queued capture chunks with an end-of-stream sentinel on stop, writes a 16-bit PCM WAV, and keeps the session warm for the next recording when options have not changed.
- WASAPI exclusive capture remains available through the existing one-shot `record-wav <output.wav>` helper path as a separate Windows coordination feature. It is not the default Handy-parity VAD path.
- Recording distinguishes the persisted Debug view from the actual developer-build launch state. The selected recorder capture mode is honored in both release and Debug views, while disabling local VAD remains a developer-build diagnostic escape hatch; packaged release builds force VAD on.
...
- Handy records differently from the original VoxType browser recorder: Handy uses native Rust/CPAL capture, native sample format selection, native worker-thread buffering, Rubato resampling, `vad-rs`, `silero_vad_v4.onnx`, 30 ms frames, and `SmoothedVad`. VoxType's shared-capture path now follows those core lifecycle and smoothing semantics; exclusive capture remains separate because it exists to coordinate with other Windows microphone consumers.
...
- Transcript history now stores successful transcription audio as WAV files under Electron's `userData` path and exposes them through IPC for playback. The saved audio is the processed WAV sent to Whisper, including local silence compaction before Whisper decoding, which makes VAD trimming and decoder-input issues audible from the history UI. Startup cleanup removes orphaned transcript audio files that are no longer referenced by retained transcript history.
...
- `src/main/transcription-service.ts` compacts long silent spans in PCM16 WAV audio, writes the prepared audio to a temp file, invokes Whisper with optional dictionary prompt context, applies local dictionary corrections, then stores corrected transcript history.
...
- `native/windows-helper` contains the first Rust native helper. It currently exposes `active-window`, `focus-window`, `capture-screenshot`, `ocr-image`, `record-wav`, `record-wav-session`, `paste-text`, `type-text`, `send-hotkey`, `mute-capture-session`, `restore-capture-session`, and `set-system-mute` commands. `active-window` returns foreground window title, hwnd, process id, process path, and process name as JSON. `focus-window` restores/focuses a captured hwnd. `capture-screenshot` captures either the full virtual screen, active foreground window, or a specific captured hwnd to PNG for the Phase 4 OCR pipeline; active-window and hwnd capture use DWM extended frame bounds instead of `GetWindowRect` to avoid DPI/window-frame offset errors. Global-hotkey OCR uses the captured target hwnd so VoxType does not accidentally OCR itself or another foreground app while recording starts. Electron keeps only the latest 10 screenshot files in the user-data screenshots folder so OCR context capture does not grow storage indefinitely. `ocr-image` uses Windows Media OCR through WinRT for fast native OCR benchmarking and tiles screenshots that exceed `OcrEngine::MaxImageDimension`, with overlap, so 4K-sized windows can be recognized without exceeding the Windows OCR bitmap limit. `record-wav-session` keeps shared capture warm for Handy-parity dictation, while `record-wav` remains the one-shot path for shared or WASAPI exclusive capture. The exclusive path negotiates hardware-friendly formats instead of assuming the shared mix format is valid. `paste-text` accepts UTF-8 text through stdin, sets the Windows Unicode clipboard, and sends Ctrl+V with `SendInput`. `type-text` accepts UTF-8 text through stdin and emits Unicode `SendInput` events so direct typing is not tied to the active keyboard layout. `send-hotkey` emits a configured accelerator through `SendInput`. `set-system-mute` uses the Windows Core Audio endpoint API to mute or unmute the default render device.
...
- Configurable global hotkeys are persisted in settings. Defaults are `CommandOrControl+Alt+Space` for dictation and hold-to-dictate, and `CommandOrControl+Shift+Space` for showing VoxType. The dictation hotkey captures the foreground window before recording, signals the renderer to start/stop microphone capture, then refocuses the captured window before paste insertion.
- Empty hotkey strings are valid persisted values. Right-clicking a hotkey button clears the setting, and the main process unregisters the previously registered shortcut before applying the new hotkey set.
- While the renderer is capturing a new hotkey, Electron main temporarily unregisters all configured global shortcuts so pressing an existing dictation/show shortcut cannot trigger app behavior during rebinding.
- Hold-to-dictate can share the same persisted key as normal dictation. A quick press starts or stops toggle dictation, while holding the dictation key longer than 700 ms records only until release. Separate hold hotkeys remain supported for existing/custom configurations. Electron registers the key-down activation, then the native Windows helper waits for the physical key combination to be released because Electron's global shortcut API does not provide a global key-up event.
...
- App startup deletes disposable Electron/Chromium renderer cache folders and VoxType-owned partial download/temp files before creating windows. Durable product data stays in explicit main-process stores such as settings, dictionary, model files, runtime installs, transcript history, and transcript audio referenced by retained history.
