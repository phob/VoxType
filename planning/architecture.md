...
- `src/renderer/src/audio-recorder.ts` captures microphone audio with an `AudioWorkletNode`, keeps the monitor path silent with zero gain, resamples to 16 kHz, optionally trims silence with VAD, and encodes WAV for Whisper. The recorder requests raw microphone audio by disabling browser echo cancellation, noise suppression, and automatic gain control, then batches worklet samples before sending them to the renderer to reduce long-recording pressure.
...
- Handy records differently from VoxType: Handy uses native Rust/CPAL capture, native sample format selection, native worker-thread buffering, and Rubato resampling. If VoxType's browser/WebAudio recorder still crackles during longer recordings, the preferred fix is to move microphone capture into the Windows helper or a dedicated native audio worker instead of continuing to harden renderer capture.
